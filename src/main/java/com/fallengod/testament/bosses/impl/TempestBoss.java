...
public class TempestBoss extends GodBoss {
    ...
    protected void setupBoss() {
        if (entity instanceof Stray stray) {
            // Set max health with null check
            if (entity.getAttribute(Attribute.MAX_HEALTH) != null) {
                ...
                plugin.getLogger().warning("MAX_HEALTH attribute is null for TempestBoss, skipping health setup");
            }
            
            // Set attack damage with null check
            if (entity.getAttribute(Attribute.ATTACK_DAMAGE) != null) {
                entity.getAttribute(Attribute.ATTACK_DAMAGE).setBaseValue(bossType.getDamage());
            } else {
                plugin.getLogger().warning("ATTACK_DAMAGE attribute is null for TempestBoss, skipping damage setup");
            }
            
            // Set movement speed with null check
            if (entity.getAttribute(Attribute.MOVEMENT_SPEED) != null) {
                entity.getAttribute(Attribute.MOVEMENT_SPEED).setBaseValue(0.3);
            } else {
                plugin.getLogger().warning("MOVEMENT_SPEED attribute is null for TempestBoss, skipping speed setup");
            }
    ...
    protected void enterPhase2() {
        ...
        // Enhanced movement and aggression with null check
        if (entity.getAttribute(Attribute.MOVEMENT_SPEED) != null) {
            entity.getAttribute(Attribute.MOVEMENT_SPEED).setBaseValue(0.4);
        } else {
            plugin.getLogger().warning("MOVEMENT_SPEED attribute is null for TempestBoss phase 2, skipping speed increase");
        }
    ...
    protected void enterPhase3() {
        ...
        // More frequent lightning and enhanced speed with null check
        if (entity.getAttribute(Attribute.MOVEMENT_SPEED) != null) {
            entity.getAttribute(Attribute.MOVEMENT_SPEED).setBaseValue(0.5);
        } else {
            plugin.getLogger().warning("MOVEMENT_SPEED attribute is null for TempestBoss phase 3, skipping speed increase");
        }
        
        // Random lightning strikes task
...
().spawnParticle(Particle.ELECTRIC_SPARK, loc, 200, 15, 15, 15, 0.5);
        entity.getWorld().spawnParticle(Particle.CLOUD, loc, 100, 10, 10, 10, 0.3);
        entity.getWorld().playSound(loc, Sound.ENTITY_LIGHTNING_BOLT_THUNDER,
